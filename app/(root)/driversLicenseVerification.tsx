...
import {
  ...
  submitDriversLicenseVerification,
...
const DriversLicenseVerification = () => {
  ...
  const hasAllDocuments = (): boolean => {
    return !!(
      parsedDocs?.frontLicense &&
      parsedDocs?.backLicense &&
      parsedDocs?.selfieWithLicense
    );
  };

  /** Handle submission of license verification */
  const handleSubmitVerification = async () => {
    // Validation
    if (!hasAllDocuments()) {
      Alert.alert(
        "Missing Documents",
        "Please upload all required documents (front, back, and selfie with license)."
      );
      return;
    }
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setIsLoading(true);
            try {
              const result = await submitDriversLicenseVerification(
                userId,
                expirationDate
              );
              ...
              if (result.success) {
                ...
                Alert.alert(
                  "Success",
                  "Your license verification has been submitted successfully! We'll review it shortly."
                );
              } else {
        ...
        <View className="flex-row items-start">
          ...
          <View className="flex-1 ml-3">
            ...
            <Text className="text-xs text-gray-600 leading-5">
              ...
              license and enter the expiration date. Make sure all photos are clear
